...
func TestLoggerWithS3Store(t *testing.T) {
	...
	g.Expect(len(req.Metadata)).To(gomega.Equal(1))
	...
	g.Expect(len(req.Annotations)).To(gomega.Equal(1))
	...
	g.Expect(len(res.Metadata)).To(gomega.Equal(1))
	...
	g.Expect(len(res.Annotations)).To(gomega.Equal(1))
