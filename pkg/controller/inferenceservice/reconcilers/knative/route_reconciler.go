...
func (r *RouteReconciler) Reconcile(isvc *v1alpha2.InferenceService) error {
	endpoint := constants.Predictor
	if isvc.Spec.Default.Transformer != nil {
		endpoint = constants.Transformer
	}

	desired := knative.NewRouteBuilder().CreateKnativeRoute(isvc, endpoint, constants.Predict)

	status, err := r.reconcileRoute(isvc, desired)
	if err != nil {
		return err
	}
	...
	if isvc.Spec.Default.Explainer != nil {
		...
		desired := knative.NewRouteBuilder().CreateKnativeRoute(isvc, endpoint, constants.Explain)

		//TODO - what about status returned?
		_, err := r.reconcileRoute(isvc, desired)
		if err != nil {
			return err
		}
	}
...
func (r *RouteReconciler) reconcileRoute(isvc *v1alpha2.InferenceService, desired *knservingv1alpha1.Route) (*knservingv1alpha1.RouteStatus, error) {
