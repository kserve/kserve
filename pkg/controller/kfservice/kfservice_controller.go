...
func (r *ReconcileService) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	desiredDefaultConfig, desiredCanaryConfig := resources.CreateKnativeConfiguration(kfsvc)

	if err := controllerutil.SetControllerReference(kfsvc, desiredDefaultConfig, r.scheme); err != nil {
		return reconcile.Result{}, err
	}

	if desiredCanaryConfig != nil {
		if err := controllerutil.SetControllerReference(kfsvc, desiredCanaryConfig, r.scheme); err != nil {
			return reconcile.Result{}, err
		}
	}

	defaultConfiguration, err := serviceReconciler.ReconcileConfiguarion(context.TODO(), desiredDefaultConfig)
	if err != nil {
		log.Error(err, "Failed to reconcile default model spec", "name", desiredDefaultConfig.Name)
	...
	if desiredCanaryConfig != nil {
		canaryConfiguration, err := serviceReconciler.ReconcileConfiguarion(context.TODO(), desiredCanaryConfig)
		if err != nil {
			log.Error(err, "Failed to reconcile canary model spec", "name", desiredCanaryConfig.Name)
			r.Recorder.Eventf(kfsvc, v1.EventTypeWarning, "InternalError", err.Error())
			return reconcile.Result{}, err
		}
