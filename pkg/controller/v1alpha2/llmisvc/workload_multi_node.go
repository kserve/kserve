...
func (r *LLMISVCReconciler) expectedMainMultiNodeLWS(ctx context.Context, llmSvc *v1alpha2.LLMInferenceService, config *Config) (*lwsapi.LeaderWorkerSet, error) {
	...
	if llmSvc.Spec.Template != nil {
		...
		if err := r.attachLoRAAdapters(ctx, serviceAccount, llmSvc, currLeaderSpec, &expected.Spec.LeaderWorkerTemplate.LeaderTemplate.Spec, config); err != nil {
			return nil, fmt.Errorf("failed to attach LoRA adapters to leader template: %w", err)
		}
	...
	if llmSvc.Spec.Worker != nil {
		...
		if err := r.attachLoRAAdapters(ctx, serviceAccount, llmSvc, currLWS.Spec.LeaderWorkerTemplate.WorkerTemplate.Spec, &expected.Spec.LeaderWorkerTemplate.WorkerTemplate.Spec, config); err != nil {
			return nil, fmt.Errorf("failed to attach LoRA adapters to worker template: %w", err)
		}
...
func (r *LLMISVCReconciler) expectedPrefillMultiNodeLWS(ctx context.Context, llmSvc *v1alpha2.LLMInferenceService, config *Config) (*lwsapi.LeaderWorkerSet, error) {
	...
	if llmSvc.Spec.Prefill != nil {
		...
		if llmSvc.Spec.Prefill.Template != nil {
			...
			if err := r.attachLoRAAdapters(ctx, serviceAccount, llmSvc, currLeaderSpec, &expected.Spec.LeaderWorkerTemplate.LeaderTemplate.Spec, config); err != nil {
				return nil, fmt.Errorf("failed to attach LoRA adapters to prefill leader template: %w", err)
			}
		}
		if llmSvc.Spec.Prefill.Worker != nil {
			...
			if err := r.attachLoRAAdapters(ctx, serviceAccount, llmSvc, currLWS.Spec.LeaderWorkerTemplate.WorkerTemplate.Spec, &expected.Spec.LeaderWorkerTemplate.WorkerTemplate.Spec, config); err != nil {
				return nil, fmt.Errorf("failed to attach LoRA adapters to prefill worker template: %w", err)
			}
		}
