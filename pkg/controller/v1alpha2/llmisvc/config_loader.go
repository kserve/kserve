...
type TemplateConfig struct {
	// RenderStrategy selects the template rendering engine. Defaults to
	// TemplateRenderStrategyJSON when unset.
	RenderStrategy TemplateRenderStrategy `json:"renderStrategy,omitempty"`
}

// NewTemplateConfig parses the "template" key from the inferenceservice-config
// ConfigMap, defaulting RenderStrategy to TemplateRenderStrategyJSON when unset.
func NewTemplateConfig(isvcConfigMap *corev1.ConfigMap) (*TemplateConfig, error) {
	...
	if template, ok := isvcConfigMap.Data[templateConfigMapKey]; ok {
		...
		switch templateConfig.RenderStrategy {
		case TemplateRenderStrategyJSON, TemplateRenderStrategyRecursive:
		default:
			return nil, fmt.Errorf("unknown template render strategy %q", templateConfig.RenderStrategy)
		}
	}

	if templateConfig.RenderStrategy == "" {
		templateConfig.RenderStrategy = TemplateRenderStrategyJSON
	}
