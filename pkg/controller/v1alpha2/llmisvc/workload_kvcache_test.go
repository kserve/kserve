...
func TestApplyKVCacheCarriers(t *testing.T) {
	...
	// env merges by name, so anything the spec declares lands on the preset's slot.
	// Filling over it would ignore the value and deleting it would make it vanish,
	// so both shapes are reported instead - and reported the same way, because the
	// user did the same thing either way.
	t.Run("a spec writing to the slot is reported, not half-written", func(t *testing.T) {
		for _, tc := range []struct {
			name   string
			mutate func(*corev1.EnvVar)
			kv     *v1alpha2.KVCacheOffloadingSpec
		}{
			{
				name: "valueFrom",
				kv:   kvSpec,
				mutate: func(e *corev1.EnvVar) {
					e.ValueFrom = &corev1.EnvVarSource{ConfigMapKeyRef: &corev1.ConfigMapKeySelector{Key: "args"}}
				},
			},
			{
				name:   "plain value",
				kv:     kvSpec,
				mutate: func(e *corev1.EnvVar) { e.Value = "--kv-transfer-config '{}'" },
			},
			{
				// Without offloading the slot is removed, so a value here would
				// disappear with nothing to explain where it went.
				name:   "plain value with no offloading configured",
				kv:     nil,
				mutate: func(e *corev1.EnvVar) { e.Value = "--kv-transfer-config '{}'" },
			},
		} {
			t.Run(tc.name, func(t *testing.T) {
				podSpec := podWithSlot()
				tc.mutate(&podSpec.Containers[0].Env[0])
				before := podSpec.DeepCopy()
				cfg := &v1alpha2.LLMInferenceServiceConfig{Spec: v1alpha2.LLMInferenceServiceSpec{
					WorkloadSpec: v1alpha2.WorkloadSpec{KVCacheOffloading: tc.kv, Template: podSpec},
				}}
				err := applyKVCacheCarriers(cfg)
				if err == nil {
					t.Fatal("want an error naming the conflict, got nil")
				}
				var slotErr *kvTransferSlotError
				if !errors.As(err, &slotErr) {
					t.Fatalf("want a *kvTransferSlotError so reconcileBaseRefs can classify it, got %T", err)
				}
				for _, want := range []string{kvTransferArgsEnvVar, mainContainerName, "VLLM_ADDITIONAL_ARGS"} {
					if !strings.Contains(err.Error(), want) {
						t.Errorf("error should name %q, got: %v", want, err)
					}
				}
				if diff := cmp.Diff(before, podSpec); diff != "" {
					t.Errorf("conflicting spec was modified (-want +got):\n%s", diff)
				}
			})
		}
	})
...
func TestParseShmPercentOfCPU(t *testing.T) {
	for _, tc := range []struct {
		raw   string
		want  int64
		wantK bool
		...
		{"1.2x", 0, false},
		// Bounded so scaleQuantity's overflow fallback stays out of reach of
		// anything a preset can carry.
		{"10000", 10000, true},
		{"10001", 0, false},
		...
		t.Run(tc.raw, func(t *testing.T) {
			got, ok := parseShmPercentOfCPU(tc.raw)
...
func TestApplyKVCacheShmSizing(t *testing.T) {
	...
	sizingFromPreset := func(cfg *v1alpha2.LLMInferenceServiceConfig, percent int64) kvCacheShmSizing {
		sizing := kvCacheShmSizing{}
		sizing.record(cfg, kvCacheShmPolicy{percentOfCPU: percent})
		return sizing
	}
	// sizeAsPreset sizes cfg as though every declared size came from a shipped
	// preset, which is what the cases below are about.
	sizeAsPreset := func(cfg *v1alpha2.LLMInferenceServiceConfig, percent int64) {
		if declared := applyKVCacheShmSizing(cfg, sizingFromPreset(cfg, percent)); len(declared) > 0 {
			t.Fatalf("nothing was user-declared, got %v", declared)
		}
	}
	...
	t.Run("adds the scaled tier to the preset baseline", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.baseline+"+"+tc.cpu, func(t *testing.T) {
				...
				sizeAsPreset(cfgWith(podSpec, tc.cpu), tc.percent)
	...
	t.Run("never touches container resources", func(t *testing.T) {
		...
		sizeAsPreset(cfgWith(podSpec, "10Gi"), 120)
	...
	// Auto-sizing is arithmetic the controller cannot check against the workload,
	// so a size the user wrote wins outright. Without this there is no way to
	// correct the math from the spec.
	t.Run("leaves a user-declared size exactly as written", func(t *testing.T) {
		podSpec := shmPod(ptr.To(resource.MustParse("64Gi")), corev1.StorageMediumMemory)
		baseline := cfgWith(shmPod(ptr.To(resource.MustParse("1Gi")), corev1.StorageMediumMemory), "10Gi")
		cfg := cfgWith(podSpec, "10Gi")
		sizing := sizingFromPreset(baseline, 120)
		sizing.record(cfg, kvCacheShmPolicy{userDeclared: true, declaredBy: "my-config"})

		declared := applyKVCacheShmSizing(cfg, sizing)

		if got := sizeLimitOf(podSpec).String(); got != "64Gi" {
			t.Errorf("sizeLimit = %s, want 64Gi", got)
		}
		if diff := cmp.Diff([]string{"template (declared by my-config)"}, declared); diff != "" {
			t.Errorf("declared workloads (-want +got):\n%s", diff)
		}
	})

	// A quantity survives the merge's JSON round trip in a different notation but
	// the same value, which must not read as a user override.
	t.Run("normalizing a preset quantity does not change its ownership", func(t *testing.T) {
		podSpec := shmPod(ptr.To(resource.MustParse("1024Mi")), corev1.StorageMediumMemory)
		baseline := cfgWith(shmPod(ptr.To(resource.MustParse("1Gi")), corev1.StorageMediumMemory), "10Gi")

		applyKVCacheShmSizing(cfgWith(podSpec, "10Gi"), sizingFromPreset(baseline, 120))

		want := resource.MustParse("13Gi")
		if got := sizeLimitOf(podSpec); got.Cmp(want) != 0 {
			t.Errorf("sizeLimit = %s, want 13Gi", got.String())
		}
	})

	// A preset that declares no size owns nothing, so a size in the merged spec
	// can only have come from the user.
	t.Run("leaves a size the presets never declared", func(t *testing.T) {
		podSpec := shmPod(ptr.To(resource.MustParse("64Gi")), corev1.StorageMediumMemory)
		baseline := cfgWith(shmPod(nil, corev1.StorageMediumMemory), "10Gi")
	...
	// Each workload is judged on its own size, or declaring one anywhere would
	// disable sizing everywhere.
	t.Run("a declared size on one workload does not stop the others", func(t *testing.T) {
		main := shmPod(ptr.To(resource.MustParse("64Gi")), corev1.StorageMediumMemory)
		worker := shmPod(ptr.To(resource.MustParse("8Gi")), corev1.StorageMediumMemory)
		cfg := cfgWith(main, "10Gi")
		cfg.Spec.Worker = worker

		baseline := cfgWith(shmPod(ptr.To(resource.MustParse("1Gi")), corev1.StorageMediumMemory), "10Gi")
		baseline.Spec.Worker = shmPod(ptr.To(resource.MustParse("8Gi")), corev1.StorageMediumMemory)
		sizing := sizingFromPreset(baseline, 120)
		sizing.record(cfgWith(main, "10Gi"), kvCacheShmPolicy{userDeclared: true, declaredBy: "my-config"})

		declared := applyKVCacheShmSizing(cfg, sizing)

		if got := sizeLimitOf(main).String(); got != "64Gi" {
			t.Errorf("template sizeLimit = %s, want 64Gi", got)
		}
		if got := sizeLimitOf(worker).String(); got != "20Gi" {
			t.Errorf("worker sizeLimit = %s, want 20Gi", got)
		}
		if diff := cmp.Diff([]string{"template (declared by my-config)"}, declared); diff != "" {
			t.Errorf("declared workloads (-want +got):\n%s", diff)
		}
