...
func TestLoadConfig_TemplateRenderer(t *testing.T) {
	tests := []struct {
		...
		{name: "explicit JSON strategy", templateData: `{"llmInferenceServiceConfigRenderStrategy":"JSON"}`, want: llmisvc.TemplateRenderStrategyJSON},
		{name: "explicit Recursive strategy", templateData: `{"llmInferenceServiceConfigRenderStrategy":"Recursive"}`, want: llmisvc.TemplateRenderStrategyRecursive},
		{name: "unknown strategy errors", templateData: `{"llmInferenceServiceConfigRenderStrategy":"bogus"}`, wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.TemplateConfig.LLMInferenceServiceConfigRenderStrategy != tt.want {
				t.Errorf("LLMInferenceServiceConfigRenderStrategy = %v, want %v", got.TemplateConfig.LLMInferenceServiceConfigRenderStrategy, tt.want)
			}
		})
	}
}
