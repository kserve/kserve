...
func ValidateStorageURI(ctx context.Context, namespace string, storageURI *string, storageContainerName *string, client client.Client) error {
	if storageURI == nil {
		return nil
	}

	// Step 1: Passes the validation if we have a storage container CR that supports this storageURI.
	storageContainerSpec, err := pod.GetContainerSpecForStorageUri(ctx, namespace, *storageURI, storageContainerName, client)
	if err != nil {
		return err
	}
