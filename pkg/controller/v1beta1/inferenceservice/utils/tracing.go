...
func injectPredictorTracing(t *v1beta1.TracingSpec, serverType string, container *corev1.Container) {
	endpoint := ptr.Deref(t.ExporterEndpoint, "")
	switch serverType {
...
// inferred from the well-known predictor and transformer container names.
// Unrecognized containers are left unchanged.
//
...
func InjectComponentTracing(t *v1beta1.TracingSpec, namespace, isvcName, variant, serverType, component string, container *corev1.Container) bool {
	...
	case string(v1beta1.PredictorComponent):
		injectPredictorTracing(t, serverType, container)
	}
