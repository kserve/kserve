...
func (p *Transformer) Reconcile(ctx context.Context, isvc *v1beta1.InferenceService) (ctrl.Result, error) {
	...
	if err := isvcutils.AddEnvVarToPodSpec(&podSpec, constants.TransformerContainerName, constants.InferenceServiceNameEnvVarKey, isvc.Name); err != nil {
		return ctrl.Result{}, errors.Wrapf(err, "failed to add INFERENCE_SERVICE_NAME environment variable to container %s", constants.TransformerContainerName)
	}
