...
func NewAIServiceBackendReconciler(client client.Client, ingressConfig *v1beta1.IngressConfig, logger logr.Logger) *AIServiceBackendReconciler {
	return &AIServiceBackendReconciler{
		...
		log:           logger.WithName("AIServiceBackendReconciler"),
	}
}
...
func (r *AIServiceBackendReconciler) Reconcile(ctx context.Context, isvc *v1beta1.InferenceService) error {
	...
	if err := r.client.Get(ctx, client.ObjectKey{Namespace: desired.Namespace, Name: desired.Name}, existing); err != nil {
		if apierr.IsNotFound(err) {
			logger.Info("Creating AIServiceBackend", "name", desired.Name, "namespace", desired.Namespace)
			if err := r.client.Create(ctx, desired); err != nil {
				logger.Error(err, "Failed to create AIServiceBackend", "name", desired.Name, "namespace", desired.Namespace)
				return err
			}
			return nil
		}
	...
	if err := r.client.Update(ctx, desired, client.DryRunAll); err != nil {
		logger.Error(err, "Failed to perform dry-run update for AIServiceBackend", "name", desired.Name, "namespace", desired.Namespace)
		return err
	}
	if !r.SemanticEquals(desired, existing) {
		logger.Info("Updating AIServiceBackend", "name", desired.Name, "namespace", desired.Namespace)
		if err := r.client.Update(ctx, desired); err != nil {
			logger.Error(err, "Failed to update AIServiceBackend", "name", desired.Name, "namespace", desired.Namespace)
			return err
		}
	}
	return nil
