...
const (
	...
	MissingPipelineParallelSizeValueError            = "pipelineParallelSize must be set (%s) when WorkerSpec is set"
	MissingTensorParallelSizeValueError              = "tensorParallelSize must be set (%s) when WorkerSpec is set"
...
func (sr *ServingRuntimeValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	for i := range ExistingRuntimes.Items {
		...
		if err := validateMultiNodeSpec(&servingRuntime.Spec); err != nil {
...
func (csr *ClusterServingRuntimeValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	for i := range ExistingRuntimes.Items {
		...
		if err := validateMultiNodeSpec(&clusterServingRuntime.Spec); err != nil {
...
func validateMultiNodeSpec(newSpec *v1alpha1.ServingRuntimeSpec) error {
