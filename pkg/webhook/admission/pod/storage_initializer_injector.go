...
// eligible StorageContainers in the namespace first, then ClusterStorageContainers,
// and returns the first match for the specified URI format.
//
// Parameters:
//   - ctx: Context for the Kubernetes API call
//   - storageUri: The storage URI to find a compatible container for (e.g., "s3://bucket/path")
//   - namespace: Namespace of the consuming workload
//   - storageContainerName: Optional name of a StorageContainer or ClusterStorageContainer
//   - c: Kubernetes client for accessing storage container resources
//
// Returns:
