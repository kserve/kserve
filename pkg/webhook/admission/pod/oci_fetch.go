...
// URI as a storage arg and the Python storage initializer's oci:// handler (added in
// Commit 2) pulls the image's model layers into a shared emptyDir volume at modelPath.
//
...
//
// Limitation (Commit 1): mixing oci+fetch:// with non-OCI storage URIs (S3/GCS/…) in the
// same pod is unsupported — the shared storage-initializer init-container name causes the
// non-OCI download path to be skipped. Single or multiple oci+fetch:// sources are supported.
