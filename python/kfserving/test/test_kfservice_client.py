...
def test_kfservice_client_watch():
    '''Unit test for kfserving watch api'''
    with patch('kfserving.api.kf_serving_client.KFServingClient.watch',
               return_value=mocked_unit_result):
        assert mocked_unit_result == KFServing.get(name='flower-sample', namespace='kubeflow')
