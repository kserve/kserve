...
import requests
from kfserving.protocols.v1_inference_handler import V1InferenceHandler
...
def test_data_plane_tensorflow_protocol_client():
    ...
    with mock.patch('requests.post', return_value=mock_resp):
        res = V1RequestHandler.predict([[1, 2]], 'http://flower.default/v1/models/flower')
        assert res == [[0.1, 0.9]]
...
def test_data_plane_tensorflow_protocol_client_raise_error():
    with mock.patch('requests.post', side_effect=
        ...
        with pytest.raises(requests.exceptions.ConnectionError):
            V1RequestHandler.predict([[1, 2]], 'http://flower.default/v1/models/flower')
