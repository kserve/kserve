...
RUN cd xgbserver && poetry version $(cat ${VERSION}) && poetry install --no-root --no-interaction --no-cache
COPY xgbserver xgbserver
RUN cd xgbserver && poetry version $(cat ${VERSION}) && poetry install --no-interaction --no-cache
