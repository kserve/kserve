...
class Tokenizer(kserve.Model):
    ...
    def preprocess(self, payload: Union[Dict, InferRequest], headers: Dict[str, str] = None) \
        ...
        infer_inputs = [InferInput(name="unique_ids", datatype='INT32', shape=list(unique_ids.shape),
                                   data=unique_ids),
                        InferInput(name="segment_ids", datatype='INT32', shape=list(segment_ids.shape),
                                   data=segment_ids),
                        InferInput(name="input_ids", datatype='INT32', shape=list(input_ids.shape),
                                   data=input_ids),
