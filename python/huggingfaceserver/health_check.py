...
def check_gpu_usage(probe_type):
    try:
        ...
        used_gpus = cluster_resources.get("GPU", 0) - cluster_resources.get("GPU_group_0", 0)

        # Determine health status based on GPU usage
        if used_gpus != available_gpus:
            print(f"{probe_type}: Unhealthy - Used: {used_gpus}, Available: {available_gpus}")
            sys.exit(1)
...
def check_registered_nodes(pipeline_parallel_size):
    try:
        ...
        if registered_node_count != int(pipeline_parallel_size):
            print(f"Readiness Probe: Unhealthy - Registered nodes count ({registered_node_count}) does not match PIPELINE_PARALLEL_SIZE ({pipeline_parallel_size}).")
            sys.exit(1)
    except Exception as e:
...
def main():
    ...
    readiness_parser = subparsers.add_parser("readiness", help="Perform readiness check")
    readiness_parser.add_argument("pipeline_parallel_size", type=int, help="Pipeline parallel size")
    ...
    registered_nodes_parser = subparsers.add_parser("registered_nodes", help="Check registered nodes")
    registered_nodes_parser.add_argument("pipeline_parallel_size", type=int, help="Pipeline parallel size")
