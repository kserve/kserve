...
def _login_from_docker_config(
    ...
    on the client BEFORE manifest/blob fetches. Silent fall-through to
    anonymous if config malformed, no matching registry entry, or auth uses
    a credential helper we can't invoke."""
    try:
    ...
    if not encoded:
        # Credential helper or unsupported auth scheme — anonymous fallback
        return
    try:
