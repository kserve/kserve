...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=0 GOOS=linux GOFLAGS=-mod=readonly go build -a -o kernelcache-manager ./cmd/${CMD}
...
COPY --from=builder /go/src/github.com/kserve/kserve/kernelcache-manager /manager
ENTRYPOINT ["/manager"]
