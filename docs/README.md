...
| ----------- | ----------- | ----------- |
| kind       | Service                       | https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#types-kinds |
| apiVersion | serving.kubeflow.org/v1alpha1 | https://kubernetes.io/docs/reference/using-api/api-overview/#api-versioning |
| metadata   | [Metadata](#Metadata)         | https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#metadata |
| spec       | [Spec](#Spec)                 | https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status |
| status     | [Status](#Status)             | https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status |
...
| spec.canary.trafficPercent | Integer                 | The amount of traffic to sent to the canary, if specified. |
...
* Execute any inspection or testing against `canary`
    * Note: The revision will be available under a specialized URI (e.g. revisionName.name.namespace.cluster.internal)
...
* Observe the KFService until the new revision progresses from "latestCreatedRevisionName" to "latestReadyRevisionName"
    * Note: `100 - trafficPercent` is still sent to "default"
* Continue incremented trafficPercent in accordance with your desired canarying strategy
* Once complete, change the key `canary` to `default` and delete the original `default` section.
* The underlying resource will swap traffic which can be observed in `status`
...
It's possible to revert to a commit the contains `canary` in the spec. The state will be reconciled to rollback to a configuration that runs both `default` and `canary`. This is possible with the data model, but somewhat of an anti-pattern for safe rollbacks. We recommend reverting to KFService spec that contains only `default`.
...
### Replicas
Though Service is intended to be a serverless data model that relies on pure auto-scaling to determine replica count, some use cases are not tolerant to the limitations of auto-scaling. It is possible to specify `spec.minReplicas` and `spec.maxReplicas`.
...
status:
  default:
    name: "my-model-endpoint-1"
    replicas: 9
    traffic: 90
  canary 
    name: "my-model-endpoint-2"
    replicas: 1
    traffic: 10
```
...
| Type        | Description |
| ----------- | ----------- | 
| Ready                | Becomes True when everything is are ready. |
| RevisionsReady       | Beomces True when the underlying revisions are ready. |
