...
# This is a helper function to make KFserving inferenceService call to a GCP cluster with IAP enabled.
...
def getToken(iap_client_id, desktop_client_id, desktop_client_secret):
    ...
    if desktop_client_id is None or desktop_client_secret is None:
        ...
        refresh_token = getRefreshTokenFromClientId(desktop_client_id, desktop_client_secret)

        token = idTokenFromRefreshToken(desktop_client_id, desktop_client_secret, refresh_token, iap_client_id)
    return token
  ...
  if input_file:
    ...
    resp = requests.post(
        ...
        headers={
            ...
            'x-goog-authenticated-user-email': 'accounts.google.com:{}'.format(user_account)
        })
  else:
    resp = requests.get(
        url,
...
def main():
  ...
  parser.add_argument('--desktop_client_id', help='The client id for Desktop OAuth client')
  parser.add_argument('--desktop_client_secret', help='The client secret for Desktop OAuth client')
  parser.add_argument('--user_account', help='The user email address which can access the namespace')
  parser.add_argument('--input', help='The input file.')
  args = parser.parse_args()

  id_token = getToken(args.iap_client_id, args.desktop_client_id, args.desktop_client_secret)
