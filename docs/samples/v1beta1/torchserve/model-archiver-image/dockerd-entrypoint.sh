...
jq -c '.[]' "$PROPERTIES_JSON" | while read -r i; do
    ...
    if [ -z "$modelFile" ]; then
        if [ -z "$extraFiles" ]; then
            torch-model-archiver --model-name "$modelName" --version "$version" --serialized-file "$MODEL_STORE/$modelName/$serializedFile" --export-path "$MODEL_STORE" --handler "$handler" -requirements-file "$requirements" --force
        else
            torch-model-archiver --model-name "$modelName" --version "$version" --serialized-file "$MODEL_STORE/$modelName/$serializedFile" --export-path "$MODEL_STORE" --extra-files "$updatedExtraFiles" --handler "$handler" -requirements-file "$requirements" --force
        fi
    else
        if [ -z "$extraFiles" ]; then
            torch-model-archiver --model-name "$modelName" --version "$version" --model-file "$MODEL_STORE/$modelName/$modelFile" --serialized-file "$MODEL_STORE/$modelName/$serializedFile" --export-path "$MODEL_STORE" --handler "$handler" -requirements-file "$requirements" --force
        else
            torch-model-archiver --model-name "$modelName" --version "$version" --model-file "$MODEL_STORE/$modelName/$modelFile" --serialized-file "$MODEL_STORE/$modelName/$serializedFile" --export-path "$MODEL_STORE" --extra-files "$updatedExtraFiles" --handler "$handler" -requirements-file "$requirements" --force
        fi
    fi
