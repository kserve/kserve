...
def fix_certificate_file(filepath, service_name):
    ...
    # Pattern 2: llmisvc-webhook-svc (no namespace before -webhook)
    pattern2 = r"- '{{ include \"{{ .Release.Namespace }}-chart.fullname\" . }}-llmisvc-webhook-svc.{{\s+.Release.Namespace }}.svc'"
