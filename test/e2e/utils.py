...
def wait_for_kfservice_ready(name, namespace='kfserving-ci-e2e-test',
    ...
    for _ in range(round(Timeout_seconds/10)):
        ...
        if status == 'True':
            return
    if debug is True:
        logging.warning("Timeout to start the KFService %s.", name)
        ...
        get_pod_log(pod='kfserving-controller-manager-0',
                    ...
                    container='manager')
    raise RuntimeError("Timeout to start the KFService %s. See above log for details.", name)


def get_pod_log(pod, namespace='kfserving-system', container=''):
    '''
    Note the arg container must be '' here, instead of None.
    Otherwise API read_namespaced_pod_log will failed if no specified container.
    '''
