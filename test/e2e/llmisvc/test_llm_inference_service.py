...
class TestCase:
    ...
    expected_gateway_class: Optional[str] = None
    expected_gateway_controller: Optional[str] = None
    ...
    def __post_init__(self):
        if self.expected_gateway_controller and not self.expected_gateway_class:
            raise ValueError(
                "GATEWAY_CLASS_NAME must be set when GATEWAY_CONTROLLER_NAME is set"
            )
    ...
    [
        pytest.param(
            TestCase(
                ...
                expected_gateway_class=os.getenv("GATEWAY_CLASS_NAME"),
                expected_gateway_controller=os.getenv("GATEWAY_CONTROLLER_NAME"),
                before_test=[
            ...
            marks=[
                ...
                pytest.mark.gateway_provider_smoke,
            ],
        ),
        pytest.param(
            TestCase(
...
def test_llm_inference_service(test_case: TestCase):  # noqa: F811
    ...
    try:
        ...
        if test_case.expected_gateway_controller:
            assert_gateway_provider_route_ready(
                kserve_client,
                test_case.llm_service,
                test_case.expected_gateway,
                test_case.expected_gateway_class,
                test_case.expected_gateway_controller,
...
def _assert_true_conditions(conditions, expected_types, resource_name):
    true_types = {
        condition.get("type")
        for condition in conditions
        if condition.get("status") == "True"
    }
    missing = set(expected_types) - true_types
    assert not missing, (
        f"{resource_name} is missing true conditions {missing}: {conditions}"
    )


def assert_gateway_provider_route_ready(
    kserve_client: KServeClient,
    llm_isvc: V1alpha1LLMInferenceService,
    gateway_name: str,
    ...
    """Verify the portable Gateway -> HTTPRoute -> InferencePool data path.

    This intentionally validates path-based Gateway API routing. Body/model-name
    routing is provider-specific and requires an AgentgatewayPolicy when using
    agentgateway.
    """
    namespace = llm_isvc.metadata.namespace
    service_name = llm_isvc.metadata.name
    route_name = f"{service_name}-kserve-route"
    pool_name = f"{service_name}-inference-pool"
    api = kserve_client.api_instance

    def assert_resources_ready():
        ...
        assert gateway.get("spec", {}).get("gatewayClassName") == gateway_class_name, (
            f"Gateway {namespace}/{gateway_name} is not backed by GatewayClass "
            f"{gateway_class_name}: {gateway.get('spec')}"
        )
        _assert_true_conditions(
            gateway.get("status", {}).get("conditions", []),
            {"Programmed"},
            f"Gateway {namespace}/{gateway_name}",
        )
        ...
        route_parent_refs = route.get("spec", {}).get("parentRefs", [])
        assert any(
            parent.get("name") == gateway_name
            and parent.get("namespace", namespace) == namespace
            for parent in route_parent_refs
        ), (
            f"HTTPRoute {namespace}/{route_name} does not reference Gateway "
            f"{namespace}/{gateway_name}: {route_parent_refs}"
        )
        backend_refs = [
            backend
        ...
        assert any(
            backend.get("group") == "inference.networking.k8s.io"
            and backend.get("kind") == "InferencePool"
            and backend.get("name") == pool_name
            for backend in backend_refs
        ), f"HTTPRoute {namespace}/{route_name} does not target {pool_name}"

        route_parents = route.get("status", {}).get("parents", [])
        matching_route_parents = [
            parent
        ...
        assert matching_route_parents, (
            f"HTTPRoute {namespace}/{route_name} has no status for "
            f"{controller_name} and Gateway {namespace}/{gateway_name}: {route_parents}"
        )
        _assert_true_conditions(
            matching_route_parents[0].get("conditions", []),
            {"Accepted", "ResolvedRefs"},
            f"HTTPRoute {namespace}/{route_name}",
        )

        pool = api.get_namespaced_custom_object(
        ...
        pool_parents = pool.get("status", {}).get("parents", [])
        matching_pool_parents = [
            parent
            for parent in pool_parents
            if parent.get("controllerName") == controller_name
            and parent.get("parentRef", {}).get("name") == gateway_name
        ]
        assert matching_pool_parents, (
            f"InferencePool {namespace}/{pool_name} has no status for "
            f"{controller_name} and Gateway {namespace}/{gateway_name}: {pool_parents}"
        )
        _assert_true_conditions(
            matching_pool_parents[0].get("conditions", []),
            {"Accepted", "ResolvedRefs"},
            f"InferencePool {namespace}/{pool_name}",
        )

        return True

    wait_for(assert_resources_ready, timeout=timeout_seconds, interval=2.0)
    logger.info(
        "Gateway provider path is ready: %s -> %s -> %s (%s)",
        gateway_name,
        route_name,
        pool_name,
        controller_name,
