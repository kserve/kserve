...
TOKENIZER_COMPONENT_LABEL = "llminferenceservice-tokenizer"
PART_OF_LABEL = "llminferenceservice"
...
class TestCase:
    ...
    resource_assertions: List[Callable[["TestCase"], None]] = field(
...
def assert_standalone_tokenizer_resources(test_case: "TestCase") -> None:
    """Verify that the controller created a standalone tokenizer Deployment and Service.

    Checks that:
    1. A Deployment with the tokenizer component label exists
    2. A ClusterIP Service with the tokenizer component label exists
    3. The scheduler Deployment does NOT contain a 'tokenizer' sidecar container
    """
    ...
    label_selector = (
        f"app.kubernetes.io/component={TOKENIZER_COMPONENT_LABEL},"
        f"app.kubernetes.io/name={service_name},"
        f"app.kubernetes.io/part-of={PART_OF_LABEL}"
    )
    ...
    assert len(deployments.items) == 1, (
        f"Expected exactly 1 tokenizer Deployment for {service_name}, "
        f"found {len(deployments.items)}"
    )

    timeout, poll_interval = 120, 5
    deploy_name = deployments.items[0].metadata.name
    ...
    while True:
        ...
        if remaining <= 0:
            raise AssertionError(
                f"Tokenizer Deployment {deploy_name} has no ready replicas "
                f"after {timeout}s (status: {deploy.status.conditions})"
            )
        logger.info(
            f"Waiting for tokenizer Deployment {deploy_name} to have ready replicas "
            f"({remaining:.0f}s remaining)"
        )
        time.sleep(min(poll_interval, remaining))
    tokenizer_deploy = deploy
    logger.info(
        f"Tokenizer Deployment {tokenizer_deploy.metadata.name} has "
        f"{tokenizer_deploy.status.ready_replicas} ready replica(s)"
    )

    services = core_v1.list_namespaced_service(namespace, label_selector=label_selector)
    assert len(services.items) == 1, (
        f"Expected exactly 1 tokenizer Service for {service_name}, "
        f"found {len(services.items)}"
    )
    tokenizer_svc = services.items[0]
    assert tokenizer_svc.spec.type == "ClusterIP", (
        f"Expected tokenizer Service type ClusterIP, got {tokenizer_svc.spec.type}"
    )
    logger.info(
        f"Tokenizer Service {tokenizer_svc.metadata.name} "
        f"(ClusterIP, ports={[p.port for p in tokenizer_svc.spec.ports]})"
    )

    scheduler_label_selector = (
        f"app.kubernetes.io/component=llminferenceservice-router-scheduler,"
        f"app.kubernetes.io/name={service_name},"
        f"app.kubernetes.io/part-of={PART_OF_LABEL}"
    )
    scheduler_deploys = apps_v1.list_namespaced_deployment(
        namespace, label_selector=scheduler_label_selector
    )
    for sched_deploy in scheduler_deploys.items:
        container_names = [c.name for c in sched_deploy.spec.template.spec.containers]
        assert "tokenizer" not in container_names, (
            f"Scheduler Deployment {sched_deploy.metadata.name} should not contain "
            f"a 'tokenizer' sidecar container, found containers: {container_names}"
        )
    logger.info("Verified scheduler Deployment(s) do not contain tokenizer sidecar")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Precise prefix KV cache routing test — also verifies standalone tokenizer
        # Deployment and Service created by the controller (UDS→vLLM render migration).
        pytest.param(
            TestCase(
                base_refs=[
                    "router-managed",
                ...
                resource_assertions=[assert_standalone_tokenizer_resources],
            ),
            marks=[
                pytest.mark.cluster_cpu,
                pytest.mark.cluster_single_node,
...
def test_llm_inference_service(test_case: TestCase):  # noqa: F811
    ...
    try:
        ...
        for assertion_fn in test_case.resource_assertions:
            assertion_fn(test_case)
    except Exception as e:
        test_failed = True
        logger.error(
