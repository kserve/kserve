...
pushd test/e2e >/dev/null
  if [[ $MARKER == "raw" && $NETWORK_LAYER == "istio-ingress" ]]; then
    ...
    pytest -m "$MARKER" --ignore=qpext --log-cli-level=INFO -n $PARALLELISM --dist worksteal --network-layer $NETWORK_LAYER --ignore=explainer/ -vv --tb=long -s --timeout 600 --reruns 2 --reruns-delay 10
  else
    pytest -m "$MARKER" --ignore=qpext --log-cli-level=INFO -n $PARALLELISM --dist worksteal --network-layer $NETWORK_LAYER -vv --tb=long -s --timeout 600 --reruns 2 --reruns-delay 10
  fi
popd
