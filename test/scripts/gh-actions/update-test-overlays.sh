...
sed -i -e "s/latest/${TAG}/g" config/overlays/test/clusterresources/kustomization.yaml

# Update controller image tag
sed -i -e "s/latest/${TAG}/g" config/overlays/test/manager_image_patch.yaml
...
sed -i -e "s/latest/${TAG}/g" config/overlays/test/llmisvc_manager_image_patch.yaml
