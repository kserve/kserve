...
func main() {
		...
		Cache: cache.Options{
			ByObject: map[client.Object]cache.ByObject{
				...
				&corev1.ConfigMap{}: {
					...
					// to r.Client.Get() calls in the controller and validator functions.
	...
	v1alpha1ConfigValidator := &v1alpha1.LLMInferenceServiceConfigValidator{
		ConfigValidationFunc:   createV1Alpha1ConfigValidationFunc(mgr.GetClient()),
	...
	v1alpha2ConfigValidator := &v1alpha2.LLMInferenceServiceConfigValidator{
		ConfigValidationFunc:   createV1Alpha2ConfigValidationFunc(mgr.GetClient()),
		WellKnownConfigChecker: wellKnownConfigChecker,
	}
...
func validateLLMISVCConfig(ctx context.Context, reader client.Reader, config *v1alpha2.LLMInferenceServiceConfig) error {
	cfg, err := llmisvc.LoadConfigFromClient(ctx, reader)
	if err != nil {
		return err
	}
